...
class TimeScraper extends BaseScraper implements TimeScraperInterface
{
    ...
    public function scrape(string|int $raceNumber, CarbonInterface|string|null $date = null): array
    {
        ...
        $crawlerUrl = sprintf($this->baseUrl, 'cyokuzen', $date, $raceNumber, '&kind=2');
        ...
        foreach (range(1, 6) as $boatNumber) {
            ...
            $racerLapTime = Normalizer::normalize($times['.col6'][$boatNumber] ?? 0.0);
            $racerTurnTime = Normalizer::normalize($times['.col7'][$boatNumber] ?? 0.0);
            $racerStraightTime = Normalizer::normalize($times['.col8'][$boatNumber] ?? 0.0);
